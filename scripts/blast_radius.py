...
HIGH_RISK_PATH_PATTERNS = [
    ...
    (r"^scripts/(validate_task|validate_closure|scan_gate|blast_radius|quota_gate|admission_gate|ci_receipts|integration_gate|impact_runner|impact_benchmark|impact_holdout|impact_promotion|receipt_mutation_benchmark|core_compatibility|harness_selftest|meta_test|policy_integrity|core_version|ultrareview_receipt|ultrareview_runner|quality_scorecard|test_impact|visual_receipt|staging_gate|worker_preflight|worker_supervisor|worker_boundary|infra_dry_run|staging_pr)\.py$", "gate-script"),
    (r"^\.pre-commit-config\.ya?ml$", "pre-commit-config"),
    (r"^scripts/(meta_test_dispatch|staging_dispatch|pr_intelligence|quality_metrics_dashboard|receipt_journal|trusted_policy|playwright_visual_producer)\.py$", "gate-script"),
]
