...
What it does (all additive — never deletes anything in the target):
    ...
    4. Writes <target>/.claude/skills/VENDORED.md with canonical provenance and
       the complete projected skill/gate inventory. This replaces stale
       `guidelines_IA` provenance left by the legacy subtree projector.
    5. Writes <target>/AGENTS.md from the generic template ONLY if the target
...
def sync_vendor_metadata(
    source: Path, target: Path, dry_run: bool,
    manifest: dict[str, str] | None = None, force: bool = False,
) -> SyncResult:
    """Write a generated provenance/inventory file for projected core files.

    Consumers historically carried ``.claude/skills/VENDORED.md`` generated by
    the old guidelines_IA subtree flow. Keeping that file stale is dangerous:
    an agent may edit the wrong upstream repository or believe that only the
    original eight skills are available. The file is generated here, included
    in the same drift manifest as every other projected artifact, and therefore
    remains self-describing after a sync.
    """
    manifest = {} if manifest is None else manifest
    relpath = ".claude/skills/VENDORED.md"
    if _is_drifted(target, manifest, relpath) and not force:
...
## Canonical gates projected into scripts/
{gate_lines}
"""
    if not dry_run:
        dst = target / relpath
        ...
        # Hash the actual destination bytes. Path.write_text uses the host's
        # newline policy on Windows, so hashing the pre-write string would
        # create false drift on every subsequent sync.
