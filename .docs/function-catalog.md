...
| `local_integration_loop.py` | `integrate(repo, base_ref, candidates, command, timeout, work_root, fetch_missing)` merges routine candidates one at a time in a disposable worktree, runs an argv-only integration command, and emits a versioned JSON/Markdown bundle holding acute, conflicting, unresolved, or failing candidates for human review. It never pushes, merges remotely, approves, deploys, or receives secrets. |
