...
| `kpi_history.py` | `build_snapshot(inputs, repository, commit, run_id, timestamp)`, `append_snapshot(journal, snapshot)`, and `summarize(journal, ...)` retain versioned numeric KPI vectors and variance statistics without raw-log storage; they never affect admission. |
