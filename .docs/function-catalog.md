...
| `impact_holdout.py` | `run_holdout_benchmark(training, holdout)` evaluates the selector against a separately hashed seeded-fault corpus and refuses `promotion_ready` for insufficient or low-recall holdout evidence; it never weakens the full-suite gate. |
| `receipt_mutation_benchmark.py` | `run_benchmark()` mutates a complete high-risk admission fixture and measures rejected/blocked/unsafe survivors; any admitted mutation fails the benchmark, but the corpus is not a proof of authenticity. |
| `receipt_fuzz_benchmark.py` | `run_benchmark(seed, case_count, max_depth)` generates bounded malformed nested authoritative fields and measures rejected/blocked/unsafe survivors; deterministic synthetic evidence is not authenticity proof. |
| `core_compatibility.py` | `evaluate(metadata, matrix)` checks consumer minimum core releases and required contract versions; incompatible or malformed matrices fail closed and do not trigger a rollout. |
