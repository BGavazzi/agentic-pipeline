...
| `local_integration_loop.py` | `integrate(...)` performs one disposable local merge pass; `run_loop(..., state_path, max_rounds, refresh)` adds bounded rediscovery and immutable-head state so repeated rounds skip only locally included candidates while keeping unresolved holds visible. Cross-repository candidates are held before ref resolution. Both emit versioned evidence and never push, merge remotely, approve, deploy, or receive secrets. |
