...
| `intent_gate.py` | `evaluate(intent, repo, base, head)` binds a versioned intent contract to exact changed files, declared effects, data classes, and negative-test evidence; it is evidence-only and never authenticates or authorizes the candidate. |
