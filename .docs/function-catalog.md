...
| `trusted_policy.py` | `evaluate(policy, attestation, base_sha, head_sha, approved_policy_ref=None)` binds protected policy and reviewer evidence to an exact commit pair and immutable pin; absent or unprotected producer evidence is blocked, never inferred as trusted. |
| `worker_preflight.py` | `evaluate(..., network_policy_verified=False)` requires self-hosted workers to present verified network-policy facts in addition to ephemeral, clean, secret-free pool facts. |
| `kpi_history.py` | `build_snapshot(inputs, repository, commit, run_id, timestamp)`, `append_snapshot(journal, snapshot)`, and `summarize(journal, ...)` retain versioned numeric KPI vectors and variance statistics without raw-log storage; the `build` CLI materializes a vector artifact; they never affect admission. |
| `admission_mutation_benchmark.py` | `run_benchmark(fixture)` applies deterministic adversarial admission mutations and reports killed/survived cases and mutation score; it never changes live admission. |
...
| `local_integration_loop.py` | `integrate(...)` performs one disposable local merge pass; `run_loop(..., state_path, max_rounds, refresh)` adds bounded rediscovery and immutable-head state so repeated rounds skip only locally included candidates while keeping unresolved holds visible. Discovery prefers remote-tracking heads and refreshes moved same-repository heads once; drafts remain held and unexecutable, while same-repository draft heads receive read-only risk/contact triage when identity resolves. Fork and unresolved candidates stay fail-closed. Unexpected worker exceptions roll back the prior disposable state; synthetic commits bypass repository hooks and child environments drop control variables. Both emit versioned evidence and never push, merge remotely, approve, deploy, or receive secrets. |
| `integration_queue.py` | `discover_all_open(...)`, `group_by_base(...)`, and `run_queue(...)` process a stacked queue by declared base ref, isolate unresolved bases as human holds, and aggregate local evidence without creating or authorizing a staging PR. |
| `intent_gate.py` | `evaluate(intent, repo, base, head)` binds a versioned intent contract to exact changed files, declared effects, data classes, and negative-test evidence; `missing_report(...)` emits fail-closed evidence; both are evidence-only and never authenticate or authorize the candidate. |
