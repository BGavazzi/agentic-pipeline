...
| **Prototype vs. production mode** (`tester`) | `prototype` (V1, the only mode actually runnable today) = type-check + lint + unit tests + "boots and responds" (health check, or `build`+`lint` for frontend, or full CDP-driven DOM assertion if `fe_real: true`). `production` (V2, not yet buildable) = adds a sandboxed DB, full regression suite, and contract tests — reserved for post-`Diamond-2` features, migrations, and RBAC. `tester` reads a repo's own tier from `.docs/conventions/repo-tiering.md` to decide — **that file is referenced but does not exist in this repo yet**; until a repo defines it, `tester` defaults conservatively to `prototype`. |
...
All four are covered by real pytest tests in `tests/test_blast_radius.py` (CI-run) — the only skill-adjacent thing in this repo with actual automated test coverage today; `meta-test` describes the target architecture for testing the *skills themselves* but hasn't been built yet.
...
One gap worth naming rather than hiding: `tester`'s `SKILL.md` reads a repo's tier from `.docs/conventions/repo-tiering.md` to decide `prototype` vs. `production` mode, but that convention file doesn't exist in this repo yet. Until it's written, `tester` falls back to its conservative default (`prototype`) — this is called out here instead of pretending the file is there.
