...
def test_run_all_scanners_isolates_one_tool_erroring(sandbox: Path, monkeypatch):
    monkeypatch.setattr(scan_gate, "run_semgrep", lambda repo, targets: "not json")
    monkeypatch.setattr(scan_gate, "run_trivy", lambda repo, targets: TRIVY_SAMPLE)
    monkeypatch.setattr(scan_gate, "run_gitleaks", lambda repo, targets: "")
...
def test_run_all_scanners_includes_dependency_check_when_enabled(sandbox: Path, monkeypatch):
    monkeypatch.setattr(scan_gate, "run_semgrep", lambda repo, targets: json.dumps({"results": []}))
    monkeypatch.setattr(scan_gate, "run_trivy", lambda repo, targets: json.dumps({"Results": []}))
    monkeypatch.setattr(scan_gate, "run_gitleaks", lambda repo, targets: "")
    monkeypatch.setattr(scan_gate, "run_dependency_check", lambda repo, task_id: DEPENDENCY_CHECK_SAMPLE)
...
def test_scan_end_to_end_blocks_on_introduced_secret(sandbox: Path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda repo, targets: json.dumps([
